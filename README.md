<div align="center">

# ⚡ Akun Instan

### Modern Digital Store & Account Marketplace

<p>
  <strong>Simple · Fast · Modern</strong><br>
  A responsive digital storefront with powerful administration tools for products, orders, users, top-ups, and custom requests.
</p>

<p>
  <a href="https://accounter.my.id/"><strong>🌐 Live Demo</strong></a> ·
  <a href="https://github.com/Kztutorial99/Account-Center/issues">🐛 Issues</a> ·
  <a href="https://github.com/Kztutorial99/Account-Center">📦 Repository</a>
</p>

<p>
  <img src="https://img.shields.io/github/stars/Kztutorial99/Account-Center?style=for-the-badge&logo=github&label=Stars" alt="GitHub Stars" />
  <img src="https://img.shields.io/github/forks/Kztutorial99/Account-Center?style=for-the-badge&logo=github&label=Forks" alt="GitHub Forks" />
  <img src="https://img.shields.io/github/issues/Kztutorial99/Account-Center?style=for-the-badge&logo=github&label=Issues" alt="GitHub Issues" />
  <img src="https://img.shields.io/github/last-commit/Kztutorial99/Account-Center?style=for-the-badge&logo=git&label=Updated" alt="Last Commit" />
</p>

<p>
  <img src="https://img.shields.io/github/repo-size/Kztutorial99/Account-Center?style=flat-square&logo=github" alt="Repository Size" />
  <img src="https://img.shields.io/badge/React-19-61DAFB?style=flat-square&logo=react&logoColor=white" alt="React" />
  <img src="https://img.shields.io/badge/Vite-8-646CFF?style=flat-square&logo=vite&logoColor=white" alt="Vite" />
  <img src="https://img.shields.io/badge/Node.js-CommonJS-339933?style=flat-square&logo=node.js&logoColor=white" alt="Node.js" />
</p>

</div>

---

## 🖼️ Preview

<div align="center">

<a href="https://accounter.my.id/">
  <img src="https://image.thum.io/get/width/1400/crop/900/https://accounter.my.id/" alt="Akun Instan Preview" width="100%" />
</a>

<br />

**🌐 Open the live store:** [accounter.my.id](https://accounter.my.id/)

</div>

> 💡 The preview above is generated from the live Akun Instan. Click it to open the application.

---

## ✨ What is Akun Instan?

**Akun Instan** is a modern web-based digital store built to provide a clean purchasing experience for customers while giving administrators a centralized dashboard to manage the entire store.

The project is designed around three principles:

> **⚡ Fast interactions · 🎨 Clean UI · 📱 Excellent mobile experience**

Whether a customer is browsing products or an administrator is processing orders and top-up requests, Akun Instan aims to keep every workflow simple, clear, and responsive.

---

## 🚀 Features

### 🛍️ Storefront

- Modern landing page and storefront
- Product browsing and product information
- Responsive product cards
- Clean purchase/request flow
- Custom account/email request support
- Mobile-friendly navigation
- Smooth UI interactions

### 📦 Order Management

- Order tracking and management
- Customer/order information
- Product and account details
- Order status management
- Confirmation dialogs for destructive actions
- Loading and action feedback

### 💳 Top Up System

- Top-up request management
- Admin approval workflow
- Top-up history
- Request status handling
- Clear success/error feedback

### 👤 User Management

- User list and account information
- User activity management
- User-related order information
- Consistent data rendering

### 🔔 Notifications

- Admin/user notification support
- Notification status handling
- Notification deletion workflow
- Clear feedback after notification actions

### 🧑‍💻 Admin Panel

Centralized administration for:

- 📊 Store data
- 📦 Orders
- 🛍️ Products
- 👥 Users
- 💳 Top-ups
- 🔔 Notifications
- 📨 Custom requests

### 📨 Custom Email / Username Requests

Users can request a custom username/email according to the available rules.

The system is designed to validate:

- Format
- Availability
- Existing records
- Duplicate requests
- Backend uniqueness

This helps administrators create requested accounts without accidentally conflicting with an existing public username/email.

### 📱 Responsive UI

Akun Instan is built to work across:

- 📱 Mobile phones
- 📲 Tablets
- 💻 Laptops
- 🖥️ Desktop screens

Layouts, cards, navigation, dialogs, tables, and forms are designed to remain usable on smaller screens.

### ⚡ UX & Feedback

Important actions should never leave the interface looking frozen or inactive.

Akun Instan uses UX patterns such as:

- Loading states
- Disabled buttons during requests
- Confirmation modals
- Success/error notifications
- Smooth transitions
- Expandable long text
- Automatic data refresh after successful actions
- Scroll-to-top navigation behavior

---

## 🧰 Tech Stack

<div align="center">

| Technology | Purpose |
| --- | --- |
| ⚛️ **React 19** | User interface and application components |
| ⚡ **Vite 8** | Development server and production build |
| 🟢 **Node.js** | Runtime/API environment |
| 🐘 **Neon Serverless** | Database connectivity |
| 🎨 **Lucide React** | Modern icon system |
| 📦 **npm** | Package management |

</div>

### Core Dependencies

```text
React 19
React DOM 19
Vite 8
@vitejs/plugin-react
@neondatabase/serverless
Lucide React
```

---

## 🏗️ Architecture

Akun Instan combines a React/Vite frontend with server-side API handlers for application data and administrative operations.

```text
Akun Instan
├── Frontend
│   ├── React
│   ├── React DOM
│   ├── Vite
│   └── Lucide React
│
├── API
│   ├── Data API
│   ├── Admin Authentication
│   └── Product Management
│
└── Database
    └── Neon Serverless
```

---

## 📊 Repository Statistics

<div align="center">

| Metric | Live Status |
| --- | --- |
| ⭐ Stars | ![Stars](https://img.shields.io/github/stars/Kztutorial99/Account-Center?style=flat-square) |
| 🍴 Forks | ![Forks](https://img.shields.io/github/forks/Kztutorial99/Account-Center?style=flat-square) |
| 🐛 Open Issues | ![Issues](https://img.shields.io/github/issues/Kztutorial99/Account-Center?style=flat-square) |
| 💾 Repository Size | ![Size](https://img.shields.io/github/repo-size/Kztutorial99/Account-Center?style=flat-square) |
| 🕒 Last Update | ![Commit](https://img.shields.io/github/last-commit/Kztutorial99/Account-Center?style=flat-square) |
| 🌿 Default Branch | `main` |
| 🔓 Visibility | `Public` |

</div>

> 📈 The badges above update automatically from GitHub, so the statistics stay current without manually editing this README.

---

## 🎨 Design Philosophy

Akun Instan aims for a visual style that feels:

- Minimal
- Modern
- Professional
- Mobile-first
- Easy to navigate
- Fast to understand

The interface should prioritize **clarity over unnecessary visual complexity** while still feeling polished and modern.

---

## 🗺️ Roadmap

- [x] Core storefront foundation
- [x] Product management
- [x] Order management
- [x] User management
- [x] Top-up management
- [x] Admin panel foundation
- [x] Responsive UI foundation
- [ ] Advanced UI/UX polish
- [ ] More comprehensive loading/skeleton states
- [ ] Expanded custom account workflow
- [ ] Additional security hardening
- [ ] Automated testing
- [ ] Performance optimization

---

## 🛠️ Development

### Requirements

- Node.js
- npm
- A Neon-compatible database configuration

### Install

```bash
git clone https://github.com/Kztutorial99/Account-Center.git
cd Akun Instan
npm install
```

### Start Development Server

```bash
npm run dev
```

### Production Build

```bash
npm run build
```

### Preview Production Build

```bash
npm run preview
```

---

## 🤝 Contributing

Contributions, ideas, bug reports, and improvements are welcome.

1. Fork the repository.
2. Create a feature branch.
3. Make your changes.
4. Test the application.
5. Commit your changes clearly.
6. Open a Pull Request.

For larger changes, please open an issue first so the proposed direction can be discussed.

---

## 🐛 Bug Reports

Found something broken?

<a href="https://github.com/Kztutorial99/Account-Center/issues/new">🚨 Open a Akun Instan Issue</a>

Please include:

- What happened
- Steps to reproduce
- Expected behavior
- Actual behavior
- Browser/device information
- Screenshots or recordings when useful

---

## 🔗 Links

| Resource | Link |
| --- | --- |
| 🌐 Live Store | https://accounter.my.id/ |
| 📦 GitHub Repository | https://github.com/Kztutorial99/Account-Center |
| 🐛 Issues | https://github.com/Kztutorial99/Account-Center/issues |
| ⭐ Star Project | https://github.com/Kztutorial99/Account-Center/stargazers |

---

## 📄 License

No explicit license file is currently included in the repository. Before redistributing or using Akun Instan commercially, please check with the project owner regarding applicable permissions.

---

<div align="center">

## ⚡ Akun Instan

**Digital commerce, made simpler.**

<br />

<a href="https://accounter.my.id/">🌐 Visit Akun Instan</a> ·
<a href="https://github.com/Kztutorial99/Account-Center">⭐ Star on GitHub</a> ·
<a href="https://github.com/Kztutorial99/Account-Center/issues">🐛 Report a Bug</a>

<br /><br />

<sub>© 2026 Akun Instan · Built by Kztutorial99</sub>

</div>
